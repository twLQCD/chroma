...
#include "inline_disco_frequencysplitting_prob_superb_w.h"
...
namespace Chroma 
{ 
  namespace InlineDiscoFreqSplitProbSuperbV2 
  {
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path, InlineDiscoFreqSplitProbSuperbV2::Params::NamedObject_t& input)
    {
    ...
    //! Propagator output
    void write(XMLWriter& xml, const std::string& path, const InlineDiscoFreqSplitProbSuperbV2::Params::NamedObject_t& input)
    {
      push(xml, path);
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path, InlineDiscoFreqSplitProbSuperbV2::Params::Param_t& param)
    {
      XMLReader inputtop(xml, path);
    ...
    //! Propagator output
    void write(XMLWriter& xml, const std::string& path, const InlineDiscoFreqSplitProbSuperbV2::Params::Param_t& param)
    {
      push(xml, path);
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path, InlineDiscoFreqSplitProbSuperbV2::Params& input)
    {
      InlineDiscoFreqSplitProbSuperbV2::Params tmp(xml, path);
      input = tmp;
    }

    //! Propagator output
    void write(XMLWriter& xml, const std::string& path, const InlineDiscoFreqSplitProbSuperbV2::Params& input)
    {
      push(xml, path);
    ...
    void exactHPE(std::map< KeyOperator_t, ValOperator_t >& db, 
		  const InlineDiscoFreqSplitProbSuperbV2::Params::Param_t& param,
    ...
    std::vector<MinCosts_t> getOptimalShifts(InlineDiscoFreqSplitProbSuperbV2::Params::Param_t& param, const multi1d<LatticeColorMatrix>& u)
    {
...
//							      param.prop.invParam);	
    ...
    const std::string name = "DISCO_FREQUENCY_SPLITTING_PROB_SUPERB";
