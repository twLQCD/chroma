...
{ 
  ...
  for (int m(0); m < p.numMom(); m++){
	...
	QDP_abort(1);
      ...
      int max_num_levels = 2*params.param.shifts.size()-1;
	...
	restart.level = 0;
	restart.noise = 0;
	num_levels = 2*params.param.shifts.size()-1;
	...
	//}
	  ...
	  //solve (D+s_{i+1}I)^{-2}
          ...
          //Handle< SystemSolver<LatticeFermion> > PP = S_rs->qprop(state_rs, params.param.prop.invParam);
	  ...
	  //std::vector<SystemSolverResults_t> res_rs = (*PP)(v_psi[1], std::vector<std::shared_ptr<const LatticeFermion>>(v_psi[1].begin(), v_psi[1].end()));
