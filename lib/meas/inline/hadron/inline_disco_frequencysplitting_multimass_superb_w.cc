...
#include "inline_disco_frequencysplitting_multimass_superb_w.h"
...
namespace Chroma 
{ 
  namespace InlineDiscoFreqSplitMMSuperb 
  {
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path, InlineDiscoFreqSplitMMSuperb::Params::NamedObject_t& input)
    {
    ...
    //! Propagator output
    void write(XMLWriter& xml, const std::string& path, const InlineDiscoFreqSplitMMSuperb::Params::NamedObject_t& input)
    {
      push(xml, path);
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path, InlineDiscoFreqSplitMMSuperb::Params::Param_t& param)
    {
      XMLReader inputtop(xml, path);
    ...
    //! Propagator input
    void read(XMLReader& xml, const std::string& path, InlineDiscoFreqSplitMMSuperb::Params& input)
    {
      InlineDiscoFreqSplitMMSuperb::Params tmp(xml, path);
      input = tmp;
    }

    //! Propagator output
    void write(XMLWriter& xml, const std::string& path, const InlineDiscoFreqSplitMMSuperb::Params& input)
    {
      push(xml, path);
    ...
    void exactHPE(std::map< KeyOperator_t, ValOperator_t >& db, 
		  const InlineDiscoFreqSplitMMSuperb::Params::Param_t& param,
	...
	const int N_rhs = param.max_rhs_interp;
        for (int k1 = 0, dk = std::min(Nsrc, N_rhs); k1 < Nsrc ; k1 += dk, dk = std::min(Nsrc - k1, N_rhs)) {
	     ...
	     X2dmatrix v_psi(param.num_shifts, X1dvector(dk));
	  ...
	  *g5v_chi[i_v] = Gamma(15) * *v_chi[i_v];
	  ...
	  for (int m1 = 0; m1 < param.num_shifts; m1++){
	      ...
	      std::vector<SystemSolverResults_t> g5res = (*PP)(g5v_psi[m1], std::vector<std::shared_ptr<const LatticeFermion>>(g5v_chi.begin(), g5v_chi.end()));
	      for (int t = 0; t < g5v_psi[m1].size(); t++){
		  *g5v_psi[m1][t] = Gamma(15) * *g5v_psi[m1][t];
	      }
...
//		  std::vector<std::shared_ptr<LatticeFermion>> v_eta(Nc * Ns * dk);
		  std::vector<std::shared_ptr<LatticeFermion>> v_eta(dk);
	    ...
	    //now need the terms with out the shift difference in front
		  ...
		  assert(g5v_psi[m1].size() == v_psi[m2].size());
		  //std::vector<std::shared_ptr<LatticeFermion>> v_eta(Nc * Ns * dk);
		  //for (int col = 0; col < v_eta.size(); col++){v_eta[col].reset(new LatticeFermion);}
		...
		//} //col
	      //} //idk
		  do_disco(dbrs[m1][m2], g5v_psi[m1], v_psi[m2], ft, param.use_ferm_state_links ? state->getLinks() : u, param.max_path_length);
	    } //m2   
    ...
    const std::string name = "DISCO_FREQUENCY_SPLITTING_MM_SUPERB";
